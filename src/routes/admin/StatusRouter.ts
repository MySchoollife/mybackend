    ...
    this.router.post(
      ...
      async (req, res, next) => {
        try {
          ...
          let { name } = req.body;
          const userType = StatusTypes.LEAD;
          name = name ? name.trim() : name;
          const nameRegex = new RegExp(`^${name}$`, "i");
          let isExists = await Status.findOne({
          ...
          if (isExists && !isExists?.is_delete) {
            return _RS.apiNew(
              res,
              false,
              "Lead Status name already used",
              {},
              startTime
            );
          }
          const user = await Status.create({ name, type: userType });

          if (req.user.type == UserTypes.TEACHER) {
            await changeLog(
              ChangeLogAction.ADD,
              `Added New Lead Status ${user?.name}.`,
              req.user.id
            );
          }
    ...
    this.router.put(
      ...
      async (req, res, next) => {
        try {
          ...
          const user = await Status.findById(id);
          ...
          let isAlready = await Status.findOne({
            name: { $regex: nameRegex },
            _id: { $ne: id },
            is_delete: false,
            type: StatusTypes.LEAD,
          });
        ...
        try {
          ...
          if (req.user.type == UserTypes.TEACHER) {
            await changeLog(
              ...
              `Changed Status Lead Status ${user?.name}.`,
              req.user.id
            );
          }
          return _RS.apiNew(
            res,
            true,
            "Lead Status status changed successfully",
            user,
            startTime
          );
        } catch (error) {
    ...
    this.router.get(
      ...
      async (req, res, next) => {
        try {
          ...
          let total = await Status.countDocuments(filter);
          ...
          let data = await Status.aggregate(pipeline).collation(
            collationOptions
          );
          ...
          return _RS.apiNew(
            res,
            true,
            "Lead Status list  get successfully",
            {
              data: adata,
              total,
              page,
